  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center pt-20 px-4 text-center">
        ...
        <p className="text-foreground dark:text-foreground mb-6">{error || "Produk tidak ditemukan"}</p>
      ...
      <nav className="sticky top-0 z-50 bg-card   border-b border-slate-200 dark:border-slate-800">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="font-heading font-bold text-lg text-foreground ">Detail Produk</span>
          </div>
          <ThemeToggle />
        </div>
      </nav>
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            <div className="relative aspect-square bg-slate-100 dark:bg-slate-800 rounded-lg overflow-hidden shadow-lg border border-slate-200/50 dark:border-slate-800/50">
              {product.image_url ? (
                <img 
          ...
          <div className="space-y-8">
            ...
            <div className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">
              ...
              <span className="text-lg text-foreground dark:text-foreground font-normal ml-2">/ item</span>
            </div>
            ...
            {(product.carbon_saved_kg > 0 || product.plastic_saved_kg > 0 || product.impact_desc) && (
              <div className="bg-gradient-to-r from-emerald-500/10 to-sky-500/10 border border-emerald-500/20 rounded-lg p-6 ">
                ...
                <div className="flex flex-wrap gap-4 mb-4">
                  {product.carbon_saved_kg > 0 && (
                    <div className="flex items-center gap-2 bg-card  px-4 py-2 rounded-lg border border-emerald-500/20">
                      ...
                      <div>
                        <div className="text-xs text-foreground dark:text-foreground">Karbon Dicegah</div>
                        <div className="font-bold text-foreground ">{product.carbon_saved_kg} kg</div>
                      </div>
                    </div>
                  )}
                  {product.plastic_saved_kg > 0 && (
                    <div className="flex items-center gap-2 bg-card  px-4 py-2 rounded-lg border border-sky-500/20">
                      <div className="bg-sky-100 dark:bg-sky-900/50 p-2 rounded-full">
                        <Droplets className="w-5 h-5 text-sky-600 dark:text-sky-400" />
                      </div>
                      <div>
                        <div className="text-xs text-foreground dark:text-foreground">Plastik Dicegah</div>
                        <div className="font-bold text-foreground ">{product.plastic_saved_kg} kg</div>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {product.impact_desc && (
                  <p className="text-sm text-foreground dark:text-foreground italic border-l-2 border-primary pl-3">
                    "{product.impact_desc}"
                  </p>
                )}
              </div>
            ...
            {/* Checkout Card */}
            <Card className="bg-card   border-slate-200 dark:border-slate-800 rounded-lg shadow-sm">
              <CardContent className="p-6 sm:p-8 space-y-6">
                ...
                {maxAllowedPoints > 0 && (
                  <div className="space-y-4 border-t border-slate-100 dark:border-slate-800 pt-6">
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-foreground ">Gunakan Eco-Points</span>
                    ...
                    <p className="text-xs text-foreground dark:text-foreground text-right">
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 pt-6 space-y-3">
                  <div className="flex justify-between text-foreground dark:text-foreground">
                  ...
                  <div className="flex justify-between text-xl font-bold text-foreground  pt-2 border-t border-dashed border-slate-200 dark:border-slate-700">
                ...
                {/* Submit Button */}
                <Button 
                  className="w-full h-14 rounded-lg text-lg font-bold shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all"
